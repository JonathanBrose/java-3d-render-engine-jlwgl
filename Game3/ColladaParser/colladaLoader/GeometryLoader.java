...
public class GeometryLoader {
	...
	public MeshData extractModelData(){
		...
		initArrays();
		float furthestPoint = convertDataToArrays();
		convertIndicesListToArray();
		
		for(Vertex v : vertices){
			if(v.getPosition().y< minY)minY = v.getPosition().y;
			else if(v.getPosition().y> maxY)maxY = v.getPosition().y;
			
		}
		
		
		return new MeshData(verticesArray, texturesArray, normalsArray, indicesArray, jointIdsArray, weightsArray,maxY-minY, furthestPoint);
	}
