...
public class EntityAnimator {
	...
	private final static float betweenAnimationLength = 0.2f;
	 ...
	 */
	public EntityAnimator(AnimatedEntity entity) {
		this.entity = entity;
	}
	 ...
	 * @param loop 
	 * 			-decides whether to loop the animation or not.
	 */
	...
	public void update() {
		...
		if(currentAnimation != nextAnimation ){
			if(!toggle){
				betweenAnimation = new Animation(betweenAnimationLength, getBetweenAnimationFrames());
				toggle=true;
				this.animationTime = 0;
				between =true;
				increaseAnimationTime();
				Map<String, Matrix4f> currentPose = calculateCurrentAnimationPose(betweenAnimation);
			...
			increaseAnimationTime();
			Map<String, Matrix4f> currentPose = calculateCurrentAnimationPose(currentAnimation);
			applyPoseToJoints(currentPose, entity.getRootJoint(), new Matrix4f());
		}
	...
	private void increaseAnimationTime() {
		...
		if(between){
			if (animationTime > betweenAnimation.getLength()) {
	 ...
	 */
	private Map<String, Matrix4f> calculateCurrentAnimationPose(Animation animation) {
		KeyFrame[] frames = getPreviousAndNextFrames(animation);
		float progression = calculateProgression(frames[0], frames[1]);
		return interpolatePoses(frames[0], frames[1], progression);
	}
	private Map<String, JointTransform> calculateCurrentAnimationPoseJoint(Animation animation) {
		KeyFrame[] frames = getPreviousAndNextFrames(animation);
		float progression = calculateProgression(frames[0], frames[1]);
		return interpolatePosesJoint(frames[0], frames[1], progression);
	}
	...
	private KeyFrame[] getPreviousAndNextFrames(Animation animation) {
		if(animation.getKeyFrames()==null)return null;
	...
	private KeyFrame[] getBetweenAnimationFrames() {
		if(currentAnimation.getKeyFrames()==null)return null;
		if(nextAnimation.getKeyFrames()==null)return null;
		KeyFrame previousFrame = new KeyFrame(0f, calculateCurrentAnimationPoseJoint(currentAnimation));
	
		KeyFrame nextFrame =  new KeyFrame(betweenAnimationLength-0.01f, nextAnimation.getKeyFrame(0).getJointKeyFrames());
	...
	private Map<String, JointTransform> interpolatePosesJoint(KeyFrame previousFrame, KeyFrame nextFrame, float progression) {
